...
use actix_web::error::{ErrorUnauthorized, ErrorBadRequest, ErrorInternalServerError};
...
impl FromRequest for BasicAuthMiddleware {
    ...
    fn from_request(req: &HttpRequest, _: &mut Payload) -> Self::Future {
        ...
        if credentials.len() != 2 {
            return ready(Err(ErrorBadRequest("Authorization header should be Basic with base64 encoded login:password")));
        }
